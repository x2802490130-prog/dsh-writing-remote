...
/** 项目位置探测：工作区根没有 novel.json 时，找一层子目录里的写文项目（优先 project/novel 目录名） */
async function resolveProjectRoot(root) {
  try { await fs.stat(path.join(root, "novel.json")); return root; } catch (e) {}
  const PRIORITY = ["project", "novel", "novel-project", "writing"];
  let entries = [];
  try { entries = await fs.readdir(root, { withFileTypes: true }); } catch (e) { return root; }
  ...
  for (const d of sorted) {
    try { await fs.stat(path.join(root, d, "novel.json")); return path.join(root, d); } catch (e) {}
  }
  return root;
}
