...
function diag(msg) {
  try { fsSync.appendFileSync("D:\\测试工具用\\remote-debug.log", "[" + new Date().toISOString() + "] " + msg + "\n"); } catch (e) {}
}
...
class WritingRemote extends TypertRemoteService {
  ...
  constructor(ctx, config) {
    ...
    diag("initializers 已执行，数量=" + initializers.length);
  }
  ...
  async projectStatus(projectRoot) {
    diag("projectStatus 收到 root=[" + projectRoot + "] 探测=[" + resolveProjectRoot(projectRoot || process.cwd()) + "]");
  ...
  async workspaceIndex(projectRoot) {
    diag("workspaceIndex 收到 root=[" + projectRoot + "] 探测=[" + resolveProjectRoot(projectRoot || process.cwd()) + "]");
    const engine = this.engineFor(projectRoot);
    let manifest = {};
    try { manifest = await engine.readManifest(); } catch (e) { return null; }
    const chapters = Array.isArray(manifest.chapters) ? manifest.chapters : [];
